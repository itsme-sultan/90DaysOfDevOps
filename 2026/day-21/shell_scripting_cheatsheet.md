...
| Take Input      | `read - p VAR`          | `echo "$VAR"`                                 |
...
- **Example**:
    ```bash
    echo $NAME # Prints Sultan
    ```
- **Quoting**:
 - **Double quotes " "**: Variable is replaced with its value.
    ```bash
    echo "Hello $NAME"   # Prints Hello Sultan
    ```
...
- **Operation** :
  - Upper case : print variable in upper case
    ```bash
    echo "${NAME^^} #prints SULTAN, all in upperr case
    ```
  - Lower case : Print variable in lower case
    ```bash
    echo "${NAME,,}" #prints sultan, all in lower case
...
- $?: Last command status
 ...
 ```bash
    ...
    ls /sultan                              # This directory does not exist
    ...
    ./script.sh sultan                      # $1 = sultan, $# = 1, $@ = sultan
...
- **Example**: Prints user IDs for root,ubuntu,and sanket
    ```bash
    for USER in root ubuntu sultan; do
        id "$USER"
    done
    ```
...
## 4.Loop control 
- **a)** break: Stops the loop immediately if condition meet.
- **Syntax**:
    ```bash
    for ITEM in list_of_items; do
...
- `while read line`
    ```bash
    ls *.txt | while read LINE; do  #output of ls will be stores in variable LINE
        echo "File: $LINE"
    done
    ```
